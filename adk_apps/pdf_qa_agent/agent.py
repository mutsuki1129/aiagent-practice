...
root_agent = Agent(
    ...
    description="A tool-using agent for summarizing and querying local PDF/CSV files.",
    instruction=(
        "你是文件問答 ADK Agent（支援 PDF/CSV），請使用繁體中文回答。\n"
        "你的任務是協助使用者理解本機文件內容，並善用工具完成推理。\n"
        "工具使用規則：\n"
        "0. 如果本輪訊息包含檔名或附件，第一步先用 set_active_document(該檔名)。\n"
        "1. 當使用者問文件大意、摘要、大綱、主要在講什麼，優先使用 summarize_document。\n"
        "2. 當使用者問指定位置內容，使用 get_document_position（PDF=頁；CSV=列）。\n"
        ...
        "6. 若不確定目前可用檔案，先用 list_available_documents 與 get_active_document。\n"
